...
$(document).ready(function () {
  ...
  $("#mrf_search").on("keyup", function () {
    var value = $(this).val().toLowerCase();

    $("#material_requset_form_purchase_table tr").filter(function () {
...
function update_reject(mrf_id, sts, cmd) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.trim() == "ok") {
        location.reload();
        // get_material_request_form_list_purchase('purchase_requested', 'all');
        // $("#material_request_form_purchase").reset()
        // $("#details_body").find("tr").eq(1).find("td").eq(0).html('')
        // $("#details_body").find("tr").eq(1).find("td").eq(1).html('')
        // $("#details_body").find("tr").eq(1).find("td").eq(2).html('')
        // $("#details_body").find("tr").eq(1).find("td").eq(3).html('')
        // $("#details_body").find("tr").eq(3).find("td").eq(0).html('')
        // $("#details_body").find("tr").eq(3).find("td").eq(1).html('')
        // $("#details_body").find("tr").eq(3).find("td").eq(2).html('')
        // $("#details_body").find("tr").eq(3).find("td").eq(3).html('')
      }
...
function insert_material_request_form_purchase() {
  ...
  if ($("#batch_table tr").length == 0) {
    if ($("#order_qty").val() == "" || parseInt($("#order_qty").val()) == 0 || $("#order_date").val() == "") {
      ...
      c_order_qty = parseInt($("#order_qty").val());
    }
  }
  else {
    $("#batch_table tr").each(function () {
      batch_details.push({ order_date: $(this).find("td").eq(1).text(), qty: $(this).find("td").eq(2).text() });
      c_order_qty = c_order_qty + parseInt($(this).find("td").eq(2).text())
    });
  }
