...
var sel_comp_cat = ""
var sub_ass_part_id = 0;
// selectAutocompleteByPartId(140)
$(document).ready(function () {
  ...
  $("#sub_ass_part_div").find("*").prop("disabled", true);
  ...
  $('#submit_btn').on('click', function () {
    ...
    console.log($("#welding_table_new tr").length);

    if (parseFloat($("#welding_table_new tr").length) > 0) {
      ...
      var inputPartsData = [];
      $('#welding_table_new tr').each(function () {
      ...
      console.log(inputPartsData, sub_ass_part_id);
      ...
      $.ajax({
        ...
        data: {
          ...
          inputPartsData: inputPartsData,
          output_part: sub_ass_part_id,
          bom_list: $('#sub_assembly_bom_list_select').find(":selected").val()
        },
        success: function (response) {
          console.log(response);
          if (response.trim() == "ok") {
      ...
      salert("Warning", "Select any parts", "warning")
    }
  ...
  $('#update_btn').on('click', function () {
    ...
    if ($('#sub_ass_part_no').data('selected-part_id') && $("#sub_assembly_bom_list_select").find(":selected").val()) {
      ...
      $.ajax({
        ...
        data: {
          ...
          inputPartsData: inputPartsData,
          output_part: $('#sub_ass_part_no').data('selected-part_id'),
          bom_id: $('#update_btn').val(),
          bom_list: $('#sub_assembly_bom_list_select').val()
        },
        success: function (response) {
          console.log(response);
          if (response.trim() == "ok") {

            get_sub_assembly_bom($('#sub_ass_part_no').data('selected-part_id'), $('#sub_assembly_bom_list_select').find(":selected").val())
            console.log($('#sub_assembly_bom_list_select').val());
  ...
  $('#add_part_btn').on('click', function () {

    {
      ...
      if ($('#part_no').val() && $('#part_name').val() && $('#qty').val() && $('#part_name').data('selected-part_id')) {
        ...
        var count = 1;
        count += $('#welding_table_new tr').length;
        console.log(count);
        alert(count)


        {
          $('#welding_table_new').append("<tr data-part-id=" + partId + " data-part-qty=" + qty + "><td>" + count + "</td><td>" + $('#part_name').val() + " </td><td contenteditable='true' class='input_part_qty' data-og_qty=" + qty + ">" + qty + "</td><td><button class='btn btn-sm btn-outline-danger border-0 m-0 p-0 px-3 delete_pr'><i class='fa fa-trash' aria-hidden='true'></i></button></td></tr>")
        }
  ...
  $("#welding_table_new").on("focusout", ".input_part_qty", function () {
    var og_qty = Number($(this).data("og_qty")) || 0;
    var entered_qty = Number($(this).text().trim());

    if (isNaN(entered_qty)) {
      $(this).text(1);
    }
    else if (entered_qty > og_qty) {
      $(this).text(og_qty);
    }
  ...
  $("#welding_table_new").on("click", "button", function (event) {
      ...
      var selected_li = $(this)
      {
        swal({
          ...
          if (isConfirm) {
            ...
            // })
            $("#welding_table_new tr").each(function (index) {
              $(this).find("td").eq(0).text(index + 1);
  ...
  $("#bom_list").on("click", "li", function () {
    ...
    var con_count = 0;
    $('#welding_table_new tr').each(function () {
      if (parseInt($(this).data("part-id")) == part_id) {
        con_count++;
      }
    })
    if (con_count > 0) {
      salert("Warning", "Part already selected", "warning");
    }
    else {
  ...
  $("#bom_recent_list").on("click", "li", function () {
    ...
    sub_ass_part_id = part_id
    get_bom_output(part_id)
    // $("#sub_ass_div").removeClass("d-none");
    $("#sub_ass_div_new").removeClass("d-none");
  });
  ...
  $('#bom_list_select').change(function () {
    ...
    console.log($('#bom_list_select').find(':selected').data('bom_id'));
    ...
    $("#sub_ass_part_div").find("*").prop("disabled", false);
  ...
  $('#sub_ass_part_name').on('input', function () {
    //check the value not empty
    if ($('#sub_ass_part_name').val() != "") {
      $('#sub_ass_part_name').autocomplete({
        //get data from database return as array of object which contain label,value
        source: function (request, response) {
          $.ajax({
            url: "php/get_part_name_auto_sub.php",
            type: "get", //send it through get method
            data: {
              term: "name",
              part: request.term,
        ...
        select: function (event, ui) {
          $(this).data("selected-part_id", ui.item.id);
          $('#sub_ass_part_no').data("selected-part_id", ui.item.id);
          $('#sub_ass_part_no').val(ui.item.part_no);
          sub_ass_part_id = ui.item.id
          get_bom_output(sub_ass_part_id)
          $("#add_sub_ass").closest(".card").find("*").prop("disabled", true);
          // $("#sub_ass_div").removeClass("d-none");
          $("#sub_ass_div_new").removeClass("d-none");
  ...
  $('#sub_ass_part_no').on('input', function () {
    //check the value not empty
    if ($('#sub_ass_part_no').val() != "") {
      $('#sub_ass_part_no').autocomplete({
        //get data from databse return as array of object which contain label,value
        ...
        select: function (event, ui) {
          ...
          $(this).data("selected-part_id", ui.item.id);
          $('#sub_ass_part_name').data("selected-part_id", ui.item.id);
          $('#sub_ass_part_name').val(ui.item.part_name)
          sub_ass_part_id = ui.item.id
          get_bom_output(sub_ass_part_id)
          $("#add_sub_ass").closest(".card").find("*").prop("disabled", true);
          // $("#sub_ass_div").removeClass("d-none");
  ...
  $("#add_bom_list_btn").on("click", function (event) {
    ...
    if (userInput && userInput.trim() !== "") {
      ...
      $("#sub_assembly_bom_list_select").append(`<option value="${userInput}">${userInput}</option>`);
      $("#sub_assembly_bom_list_select").val(userInput); // Select the newly added option
    } else {
      shw_toast("Input Required", "Please enter a BOM List Name.");
    }

  });
  $("#add_sub_ass").on("click", function (event) {

    if (($("#part_name_out").data("selected-part_id") > 0) && $("#bom_list_select").val() != null && $("#sub_ass_part_name").val() != "" && $("#sub_ass_part_no").val() != "") {
      insert_sub_ass_part()
    }
    else {
      shw_toast("Warning", "Kindly Fill All Details")
    }
...
function get_sub_assembly_bom(part_id, component_cat) {
...
function get_bom_output(part_id) {


  $.ajax({
    url: "php/get_bom_output.php",
    type: "get", //send it through get method
    data: {
      part_id: part_id
    ...
    success: function (response) {
      ...
      if (response.trim() != "error") {
        ...
        if (response.trim() != "0 result") {
          ...
          obj.forEach(function (obj) {
            count = count + 1;
            selectAutocompleteByPartId(obj.part_id)
            sel_comp_cat = obj.component_cat
            get_bom(obj.part_id, sel_comp_cat)
            get_bom_process_details(part_id, sel_comp_cat)
            get_sub_assembly_bom(part_id, $("#sub_assembly_bom_list_select").find(':selected').val())

            $('#sub_ass_part_name').data("selected-part_id", part_id);
            $('#sub_ass_part_name').val(obj.sub_part_name)

            $('#sub_ass_part_no').data("selected-part_id", part_id);
            $('#sub_ass_part_no').val(obj.sub_part_no)

            $('#sub_ass_qty').val(obj.qty)
          });
...
function insert_sub_ass_part() {


  $.ajax({
    url: "php/insert_new_sub_part.php",
    type: "get", //send it through get method
    data: {
      newPartName: $('#sub_ass_part_name').val(),
      newPartNo: $('#sub_ass_part_no').val(),
      bom_id: $('#bom_list_select').find(':selected').data('bom_id'),
      bom_cat: $('#bom_list_select').find(':selected').val(),
      bom_qty: $("#sub_ass_qty").val()
    ...
    success: function (response) {
      ...
      if (response.trim() > 0) {
        sub_ass_part_id = response.trim()
        // $("#sub_ass_part_name").prop("disabled", true);
        // $("#sub_ass_part_no").prop("disabled", true);
        $("#add_sub_ass").closest(".card").find("*").prop("disabled", true);
        // $("#sub_ass_div").removeClass("d-none");
        $("#sub_ass_div_new").removeClass("d-none");
      }
...
function get_bom_recent() {
  ...
  $.ajax({
    url: "php/get_sub_bom_process_recent.php",
    type: "get", //send it through get method
    data: {
    ...
    success: function (response) {
      ...
      if (response.trim() != "error") {
        ...
        if (response.trim() != "0 result") {
          ...
          $("#bom_list_select").val(sel_comp_cat)
      ...
      if (response.trim() != "error") {
        ...
        if (response.trim() != "0 result") {
          ...
          obj.forEach(function (obj) {
            ...
            count = count + 1;
            $("#bom_list").append("<li  data-part_id='" + obj.part_id + "' data-part_no='" + obj.part_no + "' data-part_name='" + obj.part_name + "' data-part_qty='" + obj.qty + "' class='list-group-item'>" + obj.part_name + " - <span class='fw-bold'>" + obj.qty + "</span>" + sub_ass + "</li>")
